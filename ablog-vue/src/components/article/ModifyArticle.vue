  ...
  methods: {
    loadArticleById () {
      this.$axios.post('/article', {
        ...
        this.form = resp.data
        for (let i = 0; i < resp.data.categoryList.length; i++) {
          if (resp.data.categoryList[i].categoryType === 0) {
            this.value.push(resp.data.categoryList[i].id)
          } else {
            this.tagValue.push(resp.data.categoryList[i].id)
          }
        }
        if (resp.data.cover !== '') {
          this.hasCover = true
    ...
    onSubmit (state) {
      if (this.hasComments) {
        this.form.hasComments = 100
      } else {
        this.form.hasComments = 0
      }
      if (this.tagValue.length === 0) {
        this.tagValue = '0'
      }
      this.$axios
        .post('/article/modify/' + this.value + '/' + this.tagValue, {
          id: this.id,
