    ...
    <el-form ref="form" :model="form" label-width="10px" label-position="left">
      ...
      <el-table
        ...
        <el-table-column
          ...
          width="180">
          <template slot-scope="scope">
            <span v-for="(item,key) in scope.row.categoryList" :key="key" v-if="item.categoryType === 0">{{item.categoryName}}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="发布状态"
...
export default {
  ...
  methods: {
    ...
    handleEdit (index, row) {
      this.$router.push({path: '/admin/article/modify', query: {id: row.id}})
    },
